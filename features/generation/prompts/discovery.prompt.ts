 ...
 * The agent uses this to discover visual components worth showcasing
...
export function discoveryPrompt({ playgroundDir, existingEntryIds }: DiscoveryPromptParams): string {
  ...
  return `You are scanning a React + Vite project to discover visual UI components to showcase in a design playground. The host may follow a per-route \`page.tsx\` convention (mirroring this playground's own \`page.tsx\`/\`layout.tsx\` pattern) OR be a plain Vite/React app with a single root \`App.tsx\` and no per-route files. Detect which convention applies and adapt.
...
INCLUDE:
- Single-entry convention: the root \`App.tsx\` → type \`"component"\`.
