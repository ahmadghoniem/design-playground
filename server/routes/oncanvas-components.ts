...
function scanBaseFiles(): string[] {
  return readMatchingAcrossDirs(CANVAS_COMPONENT_FILENAME_PATTERN).sort(
    (a, b) => {
...
function scanIterationFiles(): string[] {
  return readMatchingAcrossDirs(CANVAS_ITERATION_FILENAME_PATTERN).sort(
    (a, b) => {
      ...
      return (
        parseInt(parseA?.[2] ?? "0", 10) - parseInt(parseB?.[2] ?? "0", 10)
      );
    },
  );
}
...
export function oncanvasComponentsRoutes() {
  ...
  app.put("/api/oncanvas-components", async (c) => {
    ...
    const isIteration =
      filename && CANVAS_ITERATION_FILENAME_PATTERN.test(filename);

    if (!filename || (!isBase && !isIteration)) {
      return c.json(
        {
          error:
            "Invalid filename — must match frame-N.tsx or frame-N.iteration-M.tsx",
        },
        400,
      );
    }
  ...
  app.delete("/api/oncanvas-components", async (c) => {
    ...
    if (isBase) {
      ...
      const iterFiles = scanIterationFiles().filter((f) =>
        f.startsWith(`${baseName}.iteration-`),
      );
      for (const f of iterFiles) {
