...
- accordion, alert-dialog, button, chat-bits, dialog, dropdown-menu, inline-reference.tsx,
  inline-reference/ (context.tsx, dom-engine.ts), kbd, playground-nav-icons, sonner, tooltip
...
- utils, constants, element-context, generation-body, html-prompts, jsx-prompts,
  iteration-scan, model-catalog, model-icons, captureAndSaveScreenshot,
  props-fetchers.server, resolve-playground-dir, sync-host-gitignore, drag-ghost-grid
...
VERIFY:
- `grep -rn "@/components/ui/\(accordion\|button\|dialog\|tooltip\)" --include=*.ts --include=*.tsx .` → no matches (all moved to shared/ui).
