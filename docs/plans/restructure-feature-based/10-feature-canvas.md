Stack: React 19 + @xyflow/react + TS. `@/` alias + shared/ + all other features exist. Client imports are `@/`-absolute. This is the LARGEST move — the canvas core, the shape tool, and the remaining (non-iteration) node primitives.

TASK: Create `features/canvas/` and move everything canvas/shape/node-primitive related into it, updating all `@/`-import references. By this point, everything NOT moved by plans 02–09 and not staying in `app/` should end up here.
...
--- canvas hooks ---
`git mv` into `features/canvas/`:
- `hooks/useCanvasAutoArrange.ts`, `hooks/useCanvasClear.ts`,
...
- `lib/canvas-auto-arrange.ts`, `lib/canvas-flow.tsx`, `lib/canvas-paste.ts`,
  `lib/canvas-persistence.ts`, `lib/canvas-visibility.ts`, `lib/html-utils.ts`,
  `lib/jsx-utils.ts`, `lib/plan-frame-name.ts`
  → `features/canvas/<same-name>`
...
- `nodes/shared/DragSelectionOverlay.tsx`, `nodes/shared/FrameHoverHint.tsx`,
  `nodes/shared/HelperLines.tsx`, `nodes/shared/NodeLabel.tsx`, `nodes/shared/SizeButtons.tsx`
...
VERIFY:
- `grep -rn "@/app/PlaygroundCanvas\|@/components/canvas/\|@/hooks/useCanvas\|@/hooks/useElementSelection\|@/hooks/useNodeSelection\|@/hooks/useFocusNode\|@/hooks/useNodeShared\|@/lib/canvas-\|@/lib/html-utils\|@/lib/jsx-utils\|@/lib/plan-frame-name\|@/stores/interactive-node-store\|@/nodes/" --include=*.ts --include=*.tsx .` → no matches.
