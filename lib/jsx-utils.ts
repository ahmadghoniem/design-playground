 ...
 *
 * Three cases:
 * 1. Full component with `export default` → return as-is (add blank padding if needed).
 * 2. Named/arrow function without default export → add blank padding, append `export default FnName;`.
 ...
 */
export function wrapJsxComponent(
  source: string,
  ...
  const blankLinePadding = "\n\n";

  // Case 1: already has a default export — use as-is (with blank padding)
  if (
    /export\s+default\s+(function|class|const|[A-Z(])/.test(cleaned) ||
    /export\s+default\s+[a-zA-Z_$]/.test(cleaned)
  ) {
    if (!hasClientDirective) {
      return blankLinePadding + cleaned;
    }
    return cleaned;
  }
  ...
  if (anyFnMatch) {
    ...
    const body = hasClientDirective ? cleaned : blankLinePadding + cleaned;
    return `${body}\n\nexport default ${fnName};\n`;
  }

  // Case 3: bare JSX fragment — wrap in a generated function component
  const needsReactImport =
    !cleaned.includes("from 'react'") && !cleaned.includes('from "react"');
  const reactImport = needsReactImport ? "import React from 'react';\n\n" : "";
  return `${blankLinePadding}${reactImport}function ${componentName}() {\n  return (\n    <div>\n      ${cleaned.split("\n").join("\n      ")}\n    </div>\n  );\n}\n\nexport default ${componentName};\n`;
}
