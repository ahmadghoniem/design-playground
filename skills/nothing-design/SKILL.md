...
**Before starting any design work, declare which Google Fonts are required and how to load them** (see `src/app/playground/skills/nothing-design/references/tokens.md` Section 1). Never assume fonts are already available.
...
**Data status colors** (success green, warning amber, accent red) are exempt from the "one accent" rule when encoding data values. Apply color to the **value itself**, not labels or row backgrounds. See `src/app/playground/skills/nothing-design/references/tokens.md` for the full color system.
...
1. **Declare fonts** — tell the user which Google Fonts to load (see `src/app/playground/skills/nothing-design/references/tokens.md`)
...
5. **Check tokens** — consult `src/app/playground/skills/nothing-design/references/tokens.md` for exact values
6. **Build components** — consult `src/app/playground/skills/nothing-design/references/components.md` for patterns
7. **Adapt to platform** — consult `src/app/playground/skills/nothing-design/references/platform-mapping.md` for output conventions
...
- **`src/app/playground/skills/nothing-design/references/tokens.md`** — Fonts, type scale, color system (dark + light), spacing scale, grid, motion, iconography, dot-matrix motif
- **`src/app/playground/skills/nothing-design/references/components.md`** — Cards, buttons, inputs, lists, tables, nav, tags, segmented controls, progress bars, charts, widgets, overlays, state patterns
- **`src/app/playground/skills/nothing-design/references/platform-mapping.md`** — HTML/CSS, SwiftUI, React/Tailwind, Paper output conventions
