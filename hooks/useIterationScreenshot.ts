...
export function useIterationScreenshot() {
  const capture = useCallback(
    async (nodeId: string): Promise<string | null> => {
      ...
      const descRules = Object.getOwnPropertyDescriptor(
        CSSStyleSheet.prototype,
        "rules",
      );
      ...
      try {
        ...
        await toPng(frameEl, { pixelRatio: 1, width: w, height: h }).catch(
          () => null,
        );
        const url = await toPng(frameEl, {
          pixelRatio: 2,
          width: w,
          height: h,
        });
        ...
        if (descRules)
          Object.defineProperty(CSSStyleSheet.prototype, "rules", descRules);
      }
    },
    [],
  );
