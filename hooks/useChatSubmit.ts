...
export function useChatSubmit({
  ...
  const handleChatSubmit = useCallback(
    async (payload: ChatSubmitPayload) => {
      ...
      const isHtmlTarget =
        payload.renderMode === "html" && !!payload.htmlPageSlug;
      ...
      if (
            ...
            if (isHtmlTarget) {
              ...
              if (response.ok) {
                ...
                const page = pages.find(
                  (p: { folder: string }) => p.folder === payload.htmlPageSlug,
                );
                const maxNumber =
                  page?.iterations.reduce(
                    (max: number, i: { number: number }) =>
                      Math.max(max, i.number),
                    0,
                  ) ?? 0;
        ...
        try {
          ...
          if (!createRes.ok) {
            toast.error(createData?.error || "Failed to create HTML frame", {
              duration: 6000,
            });
            return;
          }
          ...
          toast.error(
            err instanceof Error ? err.message : "Failed to create HTML frame",
            { duration: 6000 },
          );
          return;
        }
