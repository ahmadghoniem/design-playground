...
export function discoveryPrompt({ playgroundDir, existingEntryIds }: DiscoveryPromptParams): string {
  ...
  return `You are scanning a React project to discover visual UI components and pages to showcase in a design playground. The host may be Next.js (App Router, has \`page.tsx\` files) OR a plain Vite/React app (a root \`App.tsx\`, no routes). Detect which and adapt.
...
INCLUDE:
- Next.js: every \`page.tsx\` (any route — auth/admin/legal/utility pages all count) → type \`"page"\`.
- Vite/React: the root \`App.tsx\` → type \`"component"\` (no route).
...
- Next.js special files: \`layout/loading/error/not-found/template/global-error.tsx\`.
