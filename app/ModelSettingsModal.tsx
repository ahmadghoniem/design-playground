...
export default function ModelSettingsModal({
  ...
  const [localClaudeOpts, setLocalClaudeOpts] =
    useState<ClaudeCodeOptions>(claudeCodeOptions);
      ...
      >
        <span
          className={`flex items-center justify-center w-4 h-4 rounded border transition-colors flex-shrink-0 ${
            checked ? "bg-stone-800 border-stone-800" : "border-stone-300"
          }`}
        >
  ...
  const toggleAll = () => {
    if (allSelected) {
      setSelected(
        new Set(selectableModels.length > 0 ? [selectableModels[0].value] : []),
      );
    } else {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col gap-1 mt-1">
          ...
          <div className="max-h-48 overflow-y-auto flex flex-col gap-0.5">
            {isLoading ? (
              <span className="text-xs text-stone-400 px-2 py-2">
                Loading models...
              </span>
            ) : (
        ...
        <p className="text-[11px] text-stone-400 px-2 -mt-1">
          Aliases track the latest release. Pinned models are under Advanced
          models. Run <code className="text-stone-500">claude update</code> for
          Opus 4.8 (v2.1.154+) and Fable 5 (v2.1.170+).
        </p>
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <span className="text-xs text-stone-600">
                  <span className="font-medium text-stone-700">
                    Detailed Claude log (stream-json)
                  </span>
                  <span className="block text-stone-500 mt-0.5">
                    Streams assistant text to the header bubble tooltip during
                    runs. Chat file download stays small (raw stream-json is not
                    written there).
                  </span>
                </span>
              </label>
