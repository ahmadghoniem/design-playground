...
`design-playground` is a **local-dev-only** design canvas that embeds into a host React app. You drag components onto an infinite canvas and use the Codex CLI to generate AI layout and style variations. **Codex (`Codex`) is the only provider.** It is never built in CI/prod — it runs only in the host's dev server.
...
- **Element selection & generation events** (added after the postMessage/scaffold era):
  ...
  - **Generation event stream**: `shared/lib/generation-events.ts` is a typed event bus (replaced ad-hoc `window` events). Iteration detection is driven by parsing `tool_use`/`tool_result` from the `Codex` stream-json output in `server/lib/Codex-jsonl.ts` (Tier 3). The client's final `scanForIterations` on generation-complete is the belt-and-braces catch for files written via `Bash` instead of `Write`/`Edit`.
...
- **Providers**: Codex only (`ProviderId = 'Codex'`). Use `DEFAULT_PROVIDER_ID` from `shared/lib/providers/registry.ts` — don't hardcode provider-id literals.
