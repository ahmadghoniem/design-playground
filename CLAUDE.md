...
- **Frontend**: React + `react-router-dom` v7, rendered by the host's Vite dev server. Entry is `page.tsx` (global CSS is imported by `dev-entry.tsx`). Tailwind v4. State via `zustand`. Canvas via `@xyflow/react`. Single-player only (solo builders). Served at the clean URL `/playground` — `server/vite-plugin.ts` rewrites `/playground` (and deep links) to the `playground.html` entry; the host's `dev-entry.tsx` react-router `basename` must be `/playground` to match.
...
- **Shared logic**: `lib/` (mostly stateless helpers: provider configs, telemetry, design-md helpers, path resolvers) — plus a few stateful subsystems (the penpal iframe bridge and the `generation-events` typed event bus). Zustand stores live in `stores/`. Server routes import from `lib/` and pass `c.req.raw` (a native Web `Request`) to helpers like `isLocalRequest` / `captureFromRequest`.
- **Element selection & generation events** (added after the postMessage/scaffold era):
  - **Iframe selection RPC** uses **penpal** (nested dep), not raw `postMessage`. `lib/iframe-bridge-types.ts` is the typed parent↔child contract; `lib/iframe-bridge.ts` (parent) and `lib/iframe-bridge-child.ts` (child, injected into the iteration iframe) implement it. Alt+hover/click selection flows through this bridge. The old `lib/iframe-selection-bridge.ts` inline bridge is gone.
  - **`data-pg-oid` stamping**: `lib/oid-stamp.ts` (parse5) stamps `data-pg-oid` attributes into on-disk HTML iterations so a selected element maps to an exact grep target for the agent. Currently HTML-only; JSX/Babel stamping is a follow-up.
  - **Generation event stream**: `lib/generation-events.ts` is a typed event bus (replaced ad-hoc `window` events). Iteration detection is driven by parsing `tool_use`/`tool_result` from the `claude` stream-json output in `server/lib/claude-jsonl.ts` (Tier 3). The old `server/lib/generation-file-watcher.ts` fs-watcher fallback has been deleted — the client's final `scanForIterations` on generation-complete is the remaining belt-and-braces catch for files written via `Bash` instead of `Write`/`Edit`. There is no more `design-system-store` — the design-system cookie is the single source of truth, read directly in `DesignSystemModal`.
  - **Iframe error badges**: `stores/iframe-error-store.ts` collects console/window errors forwarded over the bridge and surfaces them as badges on iteration nodes.
...
- **Leaf UI components are grouped under `components/<type>/`** — `modals/`, `chat/`, `canvas/` (plus the pre-existing `flow/`), and `ui/` for the shadcn primitives (`button`, `dialog`, `tooltip`, …). New leaf components should follow this convention. Note: imports are relative (`../components/ui/button`), not aliased — this package ships **no `@` alias** (no root `tsconfig`/`vite.config` to define one), so there is also no shadcn `components.json`; add new primitives by hand under `components/ui/`.
- **Shell composition files are grouped under `app/`** — `PlaygroundCanvas`, `PlaygroundClient`, `PlaygroundHeader`, `page`. Global styles live in `styles/` (`playground-global.css`). Tooling-pinned entry files stay at root: `dev-entry.tsx` (the react-router mount entry) and `registry.tsx` (imported widely across subsystems). `playground-tailwind-entry.css` also stays at root as the Vite/Tailwind build entry.
- **Provider visibility** is gated by `SHOW_ALL_PROVIDERS` in `lib/providers/registry.ts`; UI surfaces use `getVisibleProviders()`/`getVisibleProviderIds()`, and `DEFAULT_PROVIDER_ID` (also in `lib/providers/registry.ts`) is `claude-code`. Don't hardcode provider-id literals — use `DEFAULT_PROVIDER_ID`.
- Local-only endpoints (`providers`) gate on `isLocalRequest(c.req.raw)`.
- There is **no root `tsconfig.json`** — the host compiles the TS. When typechecking locally, `react`/`vite` resolve only in a host (they're peerDependencies), so module-not-found errors for those are environmental, not real. To run a *real* typecheck, do it from the host with the host's app tsconfig: `npx tsc -p tsconfig.app.json --noEmit`. Do **not** trust the host's `type-check` npm script for this — Rewynd's is a solution-style tsconfig without `--build`, so it silently checks zero files and always "passes".
