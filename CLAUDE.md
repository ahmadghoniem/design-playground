...
- `features/<name>/` — one dir per feature: `canvas` (canvas shell hooks, `nodes/`, `components/`, flow state), `discovery`, `iterations`, `generation` (owns `prompts/`), `chat`, `design-system`, `skills`. **Features never import other features** — cross-feature composition lives in `app/`; anything shared is promoted to `shared/`.
- `shared/` — `shared/ui/` (shadcn primitives + cross-feature components), `shared/lib/` (utilities used by 2+ features or by the server, incl. `providers/`, `prompts/` helpers, the iframe bridge, `generation-events`), `shared/stores/` (cross-feature zustand: `model-settings-store`, `preview-color-scheme-store`, `interactive-node-store`).
...
- Root-pinned (do not move): `dev-entry.tsx`, `registry.tsx`, `playground-tailwind-entry.css`, `setup.mjs`, `bunfig.toml`, `knip.json`, `tsconfig.json`. Content/generated dirs stay put: `iterations/`, `canvas-components/`, `skills/` (skill markdown), `assets/`, `styles/`.

The alias is defined in the package-root `tsconfig.json` (`@pg/* → ./*`, plus `@/* → ../../*` for editor IntelliSense of host imports) and registered for Vite at runtime by a `config()` hook in `server/vite-plugin.ts`. The host must add `"@pg/*": ["./src/app/playground/*"]` to its own `tsconfig.app.json` paths for host-side typechecking. **No barrel files** — features expose no `index.ts` re-export hubs; deep-import instead. Boundaries are enforced by `.dependency-cruiser.cjs` (run via a throwaway `bunx`/scratch install with `typescript@5.x` pinned — plain `bunx dependency-cruiser` is a false pass because it can't resolve `@pg/`; see `plans/restructure-feature-based/README.md`).
...
- **Element selection & generation events** (added after the postMessage/scaffold era):
  ...
  - **`data-pg-oid` stamping**: `shared/lib/oid-stamp.ts` (parse5) stamps `data-pg-oid` attributes into on-disk HTML iterations so a selected element maps to an exact grep target for the agent. Currently HTML-only; JSX/Babel stamping is a follow-up.
  ...
  - **Iframe error badges**: `features/iterations/iframe-error-store.ts` collects console/window errors forwarded over the bridge and surfaces them as badges on iteration nodes.
...
- **Shell composition files are grouped under `app/`** — `PlaygroundCanvas`, `PlaygroundClient`, `PlaygroundHeader`, `PlaygroundSidebar`, `page`. Global styles live in `styles/` (`playground-global.css`). Tooling-pinned entry files stay at root: `dev-entry.tsx` (the react-router mount entry) and `registry.tsx` (imported widely). `playground-tailwind-entry.css` also stays at root as the Vite/Tailwind build entry.
- **Provider visibility** is gated by `SHOW_ALL_PROVIDERS` in `shared/lib/providers/registry.ts`; UI surfaces use `getVisibleProviders()`/`getVisibleProviderIds()`, and `DEFAULT_PROVIDER_ID` (also in `shared/lib/providers/registry.ts`) is `claude-code`. Don't hardcode provider-id literals — use `DEFAULT_PROVIDER_ID`.
