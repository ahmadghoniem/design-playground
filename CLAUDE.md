...
- **Shell chrome**: the sidebar is permanently docked — `PlaygroundSidebar` takes no props, and the show/hide, hover-reveal, and collapse controls are gone. Don't reintroduce a toggle. The header carries preview theme, skills, and model settings; canvas reset ("clear all") and manual iteration re-scan ("refresh variations") were removed with it. Because clear-all was the only caller of `DELETE /playground/api/generate`, that cancel route is live server-side with no client caller — wire it to a stop control if you want cancellation back.
...
- **Agent**: Claude Code is the only agent CLI **today**, and the multi-provider seam is gone — there is no `providers/` directory, no `ProviderId` type, and no `DEFAULT_PROVIDER_ID`. Configuration lives in `shared/lib/agent-config.ts` (pure data + pure functions, safe to import from client code); process spawning is server-only in `server/lib/spawn-agent.ts`. A `provider` field still appears on some generation payloads for back-compat with consumers, but it is always Claude Code.
  - **Single-agent is a maintenance choice, not an architectural commitment.** More agents are planned. Do **not** pre-build a provider registry, a `ProviderId` union, or config indirection for agents that do not exist yet — that abstraction was deleted once already and re-adding it speculatively is the mistake. Instead keep the two existing seams honest: everything CLI-shaped (binary name, arg building, model catalog, not-found message) stays in `agent-config.ts`, and every process concern stays in `spawn-agent.ts`. Adding a second agent should mean touching those two files plus a picker, never a sweep through routes and components. When you find agent-specific knowledge leaking outside them — a hardcoded `claude` string, a Claude-only flag, stream-format parsing assumptions — pull it back in as part of whatever change you are already making.
  - CLI flags currently passed: `-p`, `--dangerously-skip-permissions`, `--verbose`, `--output-format` (`stream-json` + `--include-partial-messages`, or `text`), `--model`, `--effort`. **`--max-budget-usd` is deliberately not exposed** — the flag exists in the CLI, but the budget config was removed from the settings modal, the request body, and `buildAgentArgs`. Don't reintroduce it.
