...
- Per-project, never committed: `discovered-registry.json` (the component manifest) and `discovered-registry.gen.tsx` (generated from it). Both are written inside a **host** by discovery and are gitignored here. **Nothing writes them on this branch** — the static-analysis scan was removed to be redesigned (`.claude/specs/discovery-engine.md`), so `server/lib/discovered-registry.ts` keeps only `ensureModuleExists`, which writes an empty `.gen.tsx` at server boot so `registry.tsx`'s static import always resolves. A checkout with no manifest has an empty registry.
