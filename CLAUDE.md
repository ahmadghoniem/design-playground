...
- **Shell composition files are grouped under `app/`** — `PlaygroundCanvas`, `PlaygroundClient`, `PlaygroundHeader`, `PlaygroundSidebar`, `page`. Global styles live in `styles/` (`playground-global.css`, `playground-tailwind-entry.css` — the Vite/Tailwind build entry). Tooling-pinned entry files stay at root: `dev-entry.tsx` (the react-router mount entry) and `registry.tsx` (imported widely).
