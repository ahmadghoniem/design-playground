...
`design-playground` is a **local-dev-only** design canvas that embeds into a host React app. You drag components onto an infinite canvas and use an agent CLI to generate AI layout and style variations. **Claude Code (`claude-code`) is the only provider** — `ProviderId` is a single-member union and the multi-provider registry helpers are vestigial scaffolding around that one implementation. It is never built in CI/prod — it runs only in the host's dev server.
...
- **Providers**: UI surfaces use `getVisibleProviders()` / `getVisibleProviderIds()` and `DEFAULT_PROVIDER_ID` from `shared/lib/providers/registry.ts` (today always `claude-code`). Don't hardcode provider-id literals — use `DEFAULT_PROVIDER_ID`.
