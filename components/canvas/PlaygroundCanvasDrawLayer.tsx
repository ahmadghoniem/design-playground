...
function strokeBounds(stroke: DrawStroke): {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
...
export default function PlaygroundCanvasDrawLayer({
  strokes,
  wrapperRef,
}: PlaygroundCanvasDrawLayerProps) {
  const { getViewport, screenToFlowPosition } = useReactFlow();
  const [viewport, setViewport] = useState(getViewport);
  const strokeSelectEnabled = usePlaygroundDrawStore(
    (s) => s.strokeSelectEnabled,
  );
  ...
  const multiStrokeSelection = usePlaygroundDrawStore(
    (s) => s.multiStrokeSelection,
  );
  const setMultiStrokeSelection = usePlaygroundDrawStore(
    (s) => s.setMultiStrokeSelection,
  );
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (
        e.target instanceof Element &&
        e.target.closest(".react-flow__nodesselection")
      )
        return;
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (draggingRef.current) {
        setMarquee({
          sx: startRef.current.x,
          sy: startRef.current.y,
          ex: e.clientX,
          ey: e.clientY,
        });
      }
    ...
    >
      ...
      >
        <DrawStrokePaths
          ...
          selectedStrokeIds={
            multiStrokeSelection.size > 0 ? multiStrokeSelection : undefined
          }
