...
function IterationNode({ id, data, selected = false }: IterationNodeProps) {
  ...
  const { resolvedProps, isLoadingProps, propsError } = useAsyncProps(
    isHtml || isJsx ? "" : registryId,
  );
  const registryItem = useMemo(
    () => (isHtml || isJsx ? null : resolveRegistryItem(registryId)),
    [registryId, isHtml, isJsx],
  );
    ...
    try {
      ...
      if (isJsx) {
        ...
        response = await fetch("/playground/api/html-pages", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            pageFolder: data.htmlFolder,
            iterationFolder: data.htmlIterationFolder,
          }),
        });
      } else {
        ...
        <div
          ...
          className={`relative app-theme bg-background overflow-hidden rounded-xl ${isResizing ? "" : "transition-all"} ${
            adoption.adoptionStatus === "adopted"
              ...
              : selected
                ? `ring-2 ${isJsx ? "ring-purple-400" : isHtml ? "ring-orange-400" : "ring-[#0B99FF]"}`
                : ""
